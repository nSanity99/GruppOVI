...
if (!$conn->connect_error) {
    if ($user_role_from_session === 'user') {
        ...
        $sql = "SELECT id_segnalazione, data_invio, data_ultima_modifica, stato FROM segnalazioni WHERE id_utente_segnalante = ?";
        ...
        if ($stmt) {
            ...
            while ($row = $res->fetch_assoc()) {
                ...
                if (!$notif_user && $row['stato'] !== 'Inviata' && strtotime($row['data_ultima_modifica']) > strtotime($row['data_invio'])) {
                    $notif_user = true;
                }
            }
            $stmt->close();
        ...
        $sql = "SELECT id_segnalazione, stato FROM segnalazioni";
        $res = $conn->query($sql);
        $ids = [];
        if ($res) {
            while ($row = $res->fetch_assoc()) {
                if ($row['stato'] === 'Inviata') { $notif_admin = true; }
                $ids[] = $row['id_segnalazione'];
            }
        }
        if (!$notif_admin && $ids) {
